...
├── visor_videos.py        Interfaz gráfica (PySide6): carga asíncrona de la primera página + selección de carpeta + smoke test automático
...
- `main()` — bootstrap de la app + **smoke test automático**: abre la ventana, verifica el estado inicial sin carpeta, simula la selección de una carpeta temporal (diálogo inyectado y siempre restaurado), simula cancelación y verifica que se conserva la selección, espera la carga asíncrona, filtra con "real", imprime el resultado final y cierra con `exit 0`.
...
rechaza con un mensaje visible sin cerrar la ventana. **Esta etapa no
escanea la carpeta**: no detecta archivos, no abre SQLite, no ejecuta
FFprobe/FFmpeg ni genera miniaturas; la selección no es persistente y
todavía no inicia el escaneo.
