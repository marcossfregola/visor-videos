...
├── prueba_version_build.py  Pruebas automatizadas de la identificación visible de versión/build (B4.11, 3): constantes, texto exacto `Beta 4 — B4.11` y etiqueta visible en la status bar
├── configuracion.py       Servicio de persistencia de configuración (última carpeta seleccionada en `configuracion.json`) + constantes centrales de versión/build (`VERSION_PRODUCTO`, `BUILD_IDENTIFICADOR`, `TEXTO_VERSION_BUILD` → `Beta 4 — B4.11`)
