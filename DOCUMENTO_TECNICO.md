...
├── visor_videos.py        Interfaz gráfica (PySide6): carga asíncrona de la primera página + selección de carpeta + escaneo asíncrono de la carpeta elegida + encadenamiento escaneo → registros básicos → guardado + smoke test automático
...
- `iniciar_escaneo()` — acción manual del botón "Escanear carpeta": si el gestor está ocupado retorna; revalida la carpeta con `os.path.isdir`; crea una `TareaEscaneo(carpeta)` y la inicia con el **mismo** `GestorTareas` de la ventana (reutilizado para la carga inicial y para los escaneos sucesivos); marca `_escaneo_pendiente = True`, resetea el estado del guardado (`_guardado_pendiente = False`, `registros_guardados = None`, `tarea_guardado = None`), guarda la tarea en `tarea_escaneo` y muestra `MENSAJE_ESCANEANDO` ("Escaneando carpeta…"). Si `gestor.iniciar()` rechaza la tarea, vuelve al estado anterior. **El escaneo encadena el guardado**: al terminar el escaneo se preparan registros básicos y se inicia `TareaGuardarVideos` con el mismo gestor.
- `_al_resultado_escaneo(videos)` — al recibir la lista, la copia en `videos_detectados`, limpia `_escaneo_pendiente`, **marca `_guardado_pendiente = True`** (para que el resultado/error siguiente pertenezca al guardado) y muestra el conteo en `estado_escaneo` ("1 video detectado" / "N videos detectados"). No crea tarjetas ni recarga el catálogo.
...
- `_al_tarea_finalizada()` — **dispara el guardado al terminar el escaneo**: si `_guardado_pendiente` está activo y el gestor vuelve a `inactivo` tras la finalización del escaneo (el gestor solo admite una tarea a la vez), prepara los registros con `preparar_registros_basicos(videos_detectados, tarea_escaneo.carpeta)` y los persiste con `TareaGuardarVideos(registros, ruta_db)` iniciada con el mismo `GestorTareas`; si el inicio falla, limpia `_guardado_pendiente`. No se ejecuta FFprobe/FFmpeg ni se generan miniaturas en este paso.
- `_al_resultado_guardado(resultado)` — al recibir `{"guardados": n, "nombres": [...]}`: limpia `_guardado_pendiente`, guarda la cantidad en `registros_guardados` y habilita de nuevo el botón de escaneo (gestor `inactivo`). No crea tarjetas ni recarga el catálogo.
...
- Enrutado por estado: `_al_resultado` / `_al_error` reenvían el resultado/error a los handlers de escaneo (`_al_resultado_escaneo` / `_al_error_escaneo`) **cuando `_escaneo_pendiente` está activo**, y a los de guardado (`_al_resultado_guardado` / `_al_error_guardado`) **cuando `_guardado_pendiente` está activo**; la cadena se produce porque el escaneo y el guardado son tareas sucesivas con el mismo gestor (el guardado se lanza al recibir `tarea_finalizada` del escaneo, no en el handler del resultado). Es suficiente para una única tarea activa a la vez y debe revisarse si la interfaz incorpora más tipos de tarea. **Ausencia deliberada**: el pipeline limitado escribe registros **básicos** (nombre, ruta absoluta, extensión, fecha de importación) mediante el upsert transaccional existente; no recorre subcarpetas, no ejecuta FFprobe/FFmpeg, no genera miniaturas, no elimina registros ausentes, no crea tarjetas y no recarga el catálogo; no constituye la sincronización completa.
...
- `main()` — bootstrap de la app + **smoke test automático**: crea una **base SQLite temporal** reutilizando el esquema existente (`conectar_bd(ruta_db)` + `commit()` + `close()`, sin depender de `biblioteca.db`), abre la ventana, verifica el estado inicial sin carpeta ("Sin escanear" y botón de escanear deshabilitado), simula la selección de una carpeta temporal (diálogo inyectado y siempre restaurado) con archivos de video y no-video, simula cancelación y verifica que se conserva la selección, espera la carga asíncrona, dispara el escaneo real con `boton_escanear.click()`, observa "Escaneando carpeta…" mientras el gestor está ocupado, espera el resultado, imprime `videos_detectados` (3 videos), la cantidad de registros guardados (`guardado_total=3`), el estado final y el filtro con "real", y cierra con `exit 0`.
...
Capa de **tareas asíncronas**: no define lógica de catálogo ni de datos. Re-exporta desde `escanear_videos.py` las funciones que la interfaz necesita importar (entre ellas `preparar_registros_basicos` y `conectar_bd`), lo que evita que `visor_videos.py` dependa directamente del backend de catálogo.
...
| Encadenamiento del pipeline limitado desde la interfaz | `visor_videos.VisorVideos` + `tareas_videos.TareaEscaneo`/`TareaGuardarVideos` + `escanear_videos.preparar_registros_basicos` | Tareas sucesivas con el mismo `GestorTareas`: `TareaEscaneo` → preparación de registros básicos → `TareaGuardarVideos`. El guardado se lanza al recibir `tarea_finalizada` del escaneo (el gestor `Ocupado` rechaza una segunda tarea mientras otra corre); resultado/error del guardado limpian `_guardado_pendiente`; tras un error de guardado el gestor queda `inactivo` y un nuevo escaneo es posible. No es la sincronización completa del catálogo. |
...
4. `_al_resultado` reenvía el resultado a `_al_resultado_escaneo`
   ...
   `_guardado_pendiente = True`** y muestra el conteo ("1 video
...
5. Ante un error (carpeta inexistente, ruta-archivo), `_al_error_escaneo`
   muestra "No se pudo escanear la carpeta" y **conserva el último
   resultado exitoso** en `videos_detectados`; la cadena de guardado no
   se inicia.

Encadenamiento del guardado (tareas sucesivas con el mismo gestor):

6. Al terminar el hilo del escaneo, `GestorTareas` vuelve a `inactivo` y
   emite `tarea_finalizada`. `_al_tarea_finalizada()` detecta
   `_guardado_pendiente` activo y el gestor `inactivo`, prepara los
   registros con `preparar_registros_basicos(videos_detectados,
   tarea_escaneo.carpeta)` (claves `{nombre, ruta, extension,
   fecha_importacion}` con ruta absoluta) y los persiste con
   `TareaGuardarVideos(registros, ruta_db)` iniciada con el mismo
   `GestorTareas`. El guardado no se lanza en el handler del resultado
   del escaneo: el gestor `Ocupado` rechazaría una segunda tarea.
7. En el hilo de trabajo, `TareaGuardarVideos._trabajo()` invoca
...
8. `_al_resultado` reenvía el resultado a `_al_resultado_guardado`
   (enrutado por `_guardado_pendiente`), que limpia el flag, guarda la
   cantidad en `registros_guardados` y habilita de nuevo el botón de
   escaneo. No se crean tarjetas ni se recarga el catálogo.
9. Ante un error de escritura, `_al_error_guardado` limpia
...
10. El pipeline limitado **no constituye la sincronización completa**:
    no ejecuta FFprobe (los registros quedan sin duración, resolución ni
    codec), no genera miniaturas, no elimina registros ausentes, no
    recarga la grilla ni recorre subcarpetas.
...
| 13 | Baja | El pipeline limitado escribe registros **básicos** (nombre, ruta absoluta, extensión, fecha de importación) sin ejecutar FFprobe; los videos quedan sin duración, resolución ni codec hasta que FFprobe se integre en el pipeline. Conocida y limitada a esta etapa. |
...
3. **Media — Trabajos en segundo plano**: **en curso** — infraestructura implementada en `tareas.py` con `TareaFFprobe`, `TareaEscaneo`, la lectura del catálogo (`TareaLecturaCatalogo`), la **escritura individual** (`TareaGuardarVideo`) y la **escritura de colección** (`TareaGuardarVideos`). **Integración con la ventana**: la carga inicial asíncrona de la primera página del catálogo ya está integrada en `visor_videos.py` y el **pipeline limitado** (`TareaEscaneo` → `preparar_registros_basicos` → `TareaGuardarVideos`) ya se encadena desde la interfaz. Pendiente: la sincronización completa del catálogo (escritura masiva con FFprobe en el pipeline y eliminación de registros ausentes), FFmpeg asíncrono y barra de progreso.
