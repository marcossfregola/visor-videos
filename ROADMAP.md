...
8. **B6.8 — Motor general y reutilizable de nombres.** **Completada, implementada, probada y validada manualmente; publicada en `beta6` (2026-08-20, este commit).** Componente puro y testeable `nombres.py` separado de UI/SQLite/FFmpeg/PySide6 (solo stdlib) con plantillas de tokens cerrados `{original}/{numero}/{fecha}/{texto}/{inicio}/{fin}` (validación de tokens/formatos, sin eval), sanitización Windows (reemplazo `<>:"/\|?*` y controles U+0000..U+001F por `_`, rstrip `" ."`, prefijo `_` para reservados CON/PRN/AUX/NUL/COMx/LPTx, Unicode preservado, vacío→error, longitud máxima 255 con validación explícita para extensión y sufijo sin truncado silencioso), extensión controlada aparte (`validar`/`normalizar`/`asegurar_extension` con whitelist cerrada `{.mp4,.mkv}` — `.avi`/`.exe` explícitos rechazados con `ExtensionInvalidaError`, sin extensión agrega default `.mp4`, `.MP4`/`.MKV` normalizados a minúsculas), resolución determinista de colisiones FS/intra-lote sin sobrescritura (`_001`, `_002`… case-insensitive, considerando simultáneamente `existe_fn` y conjunto de lote, sin creación de archivos ni `os.replace`/`-y`); integración B6.7 delega sugerencia/extensión al motor (`generar_sugerencia_exportacion` + `asegurar_extension`), eliminando duplicación en `visor_videos.py`. Suite `prueba_nombres_b68.py` **25/25** y regresiones `prueba_exportacion_b67.py` **21/21**, `prueba_filtro_b65.py` **24/24** en verde; **evidencia humana: Marcos completó la validación manual real — al intentar guardar con extensión `.avi` la aplicación informó correctamente que la extensión no era válida y canceló la operación**. Preparado para reutilizarse en Beta 7 para renombrado masivo. No cierra la Beta 6; próxima etapa **B6.9 — Exportación múltiple de segmentos separados** (selección explícita/clasificación, motor B6.8, progreso por lote, resultados parciales seguros).
9. **B6.9 — Exportación múltiple de segmentos separados.** **Completada, implementada, probada y validada manualmente; cerrada en commit previo `1bfeda2` (2026-08-20).** Seleccionar varios segmentos y producir un archivo independiente por segmento: selección explícita y por clasificación (p. ej. "todos los segmentos verdes") con whitelist; uso del motor B6.8; progreso por lote; resultados parciales seguros; sin confundir fallo individual con éxito total. Suite `prueba_exportacion_lote_b69.py` **30/30** y regresiones B6.7 **21/21**, B6.8 **25/25** en verde.
10. **B6.10 — Unión de varios segmentos del mismo original.** **Completada, implementada, probada y validada manualmente; cerrada en este commit (2026-08-21).** Producir un único video compuesto por varios segmentos del **mismo** original en **orden explícito** definido por el usuario: **vía principal recodificada CPU con `trim`/`atrim`/`setpts`+`concat` (`libx264 veryfast crf18 yuv420p` + `aac 128k`, `+faststart` en MP4) para precisión independiente de keyframes; fallback por extracción precisa de cada segmento + `concat` demuxer para fuentes con subtítulos compatibles (`mov_text` en MP4, `srt` en MKV, mapeo explícito `0:v/0:a/0:s`); rechazo claro de MKV/SubRip no validado; FFprobe previo, temporal único en mismo directorio, publicación atómica sin `-y`, sin sobrescritura, cancelación real con limpieza y verificación final.** No extender a unión arbitraria de videos distintos. Suites `prueba_exportacion_secuencia_b610.py` **15/15**, `prueba_ui_export_fix_b610.py` **10/10** y regresiones B6.7 **21/21**, B6.8 **25/25**, B6.9 **30/30** en verde; validación humana de Marcos OK. **Beta 6 continúa abierta; siguiente etapa B6.11.**
11. **B6.11 — Incorporación al catálogo y trazabilidad de videos derivados.** **Completada, auditada y cerrada en este commit (2026-08-21, rama `beta6`). Contrato demostrado:** alta incremental aun fuera de raíz sin reescaneo completo; trazabilidad original→derivado y segmentos ordenados (`videos_derivados` con snapshot `original_nombre/ruta`/`derivado_nombre/ruta` + `videos_derivados_segmentos` con `orden`/`segmento_id`/`inicio`/`fin`); snapshot histórico sin `CASCADE` destructivo (persiste tras borrar original o derivado de `videos`); bloqueo derivado-de-derivado; fallo de catalogación conserva archivo (`catalog_error`); validación estricta de secuencia (longitud y correspondencia `segmentos` vs `segmentos_info_orden`/`orden`); rechazo de nombre duplicado `UNIQUE(nombre)` sin reutilización silenciosa; tablas `videos_derivados`/`videos_derivados_segmentos` con migración aditiva idempotente `CREATE IF NOT EXISTS`; APIs `incorporar_video_derivado_al_catalogo`/`obtener_derivacion_por_derivado`/`listar_derivaciones_por_original`/`es_video_derivado` + integración en `TareaExportarSegmento`/`TareaExportarLoteSegmentos`/`TareaExportarSecuencia` con `alta_catalogo`; suite `prueba_derivados_b611.py` **15/15** y regresiones B6.7 **21/21**, B6.9 **30/30**, B6.10 **15/15** en verde. Ver `HISTORIAL_PROYECTO.md` ##113.
12. **B6.12 — Integración, robustez y cierre funcional.** **Cerrada en rama `beta6` (commit `7d85e94bb8b617209a155e5b1086d1d38f4784f8`; B6.1–B6.12 completas; identidad `Beta 6 - B6.12`; packaging reproducible y validación real del instalador aprobada —instalación/desinstalación/reinstalación aislada preservando `biblioteca.db`/`configuracion.json`/`miniaturas`—; tag `v6.0-beta` anotado publicado sobre `7d85e94`, `origin/beta6` alineado y GitHub Release Beta 6 prerelease sin binarios).** Validación integrada completa de todos los bloques anteriores: **integración 14/14** (`prueba_integracion_b612.py`: filtro marcador/color+orden/paginación, segmento Sin clasificar, resumen colapsado batch, export individual real FFmpeg/FFprobe+naming+alta/trazabilidad, lote por color/colisiones/parciales, secuencia ordenada+trazabilidad exacta, cancelación sin temporales/trazabilidad falsa, derivado fuera de raíz, persistencia/snapshot histórico, bloqueo derivado-de-derivado, `UNIQUE(nombre)/catalog_error`, migraciones idempotentes+`PRAGMA integrity_check=ok`, filtro texto+color AND, derivado eliminado histórica), **reescaneo 3/3** (`prueba_reescaneo_preserva_metadatos_b612.py`: base/modificado/plan completo con APIs productivas `preparar_registros_basicos`/`combinar_registros_con_ffprobe/_miniaturas/_tamanos`/`guardar_videos`/`detectar_diferencias`/`preparar_plan_sincronizacion`/`aplicar_incorporaciones`/`eliminar_candidatos`, IDs y marcadores/segmentos/colores preservados, `integrity_check=ok`), **B6.11 15/15 de control** (`prueba_derivados_b611.py`) con **FFmpeg 8.1.1/FFprobe 8.1.1 reales** y **validación humana de persistencia tras reinicio+reescaneo**. Observación no bloqueante: incidente aislado anterior no reproducido en los 3 escenarios. **Beta 6 cerrada y publicada; tag `v6.0-beta` anotado publicado sobre `7d85e94`, `origin/beta6` alineado y GitHub Release Beta 6 prerelease sin binarios.** Ver `HISTORIAL_PROYECTO.md` ##115.
...
# Beta 7 — "Organización y operaciones de archivos" (cerrada y publicada en B7.13)
...
> **B7.1–B7.13 — Completadas, cerradas y publicadas (2026-08-23, commit oficial `f9976d3b3b68a197bf8e9d29a4ecc670f48a9709` `B7 Cerrar Beta 7 B7.13`, identidad `Beta 7 - B7.13`; tag `v7.0-beta` → `f9976d3`; rama `beta7` publicada y puede contener reconciliaciones documentales posteriores al tag):** B7.1 renombrado individual, B7.2 mover, B7.3 crear carpeta, B7.4 copiar, B7.5 eliminar, B7.6 lote, B7.7 renombrado masivo, B7.8 consistencia post-operaciones, B7.9 modo Organización, B7.10 navegación destino/Explorer, B7.11 doble panel, B7.12 objetivo drop, B7.13 drag & drop conectado a movimiento real con prevalidación atómica (IDs + rutas fuente + destino). **No se crea B7.14 funcional.** Beta 7 funcionalmente cerrada en B7.13 y publicada: **commit oficial `f9976d3` con tag `v7.0-beta` → `f9976d3`; rama `beta7` publicada y puede contener reconciliaciones documentales posteriores; repositorio GitHub actualmente PUBLIC, GitHub Release `v7.0-beta` prerelease publicada sin binarios**; **validación específica del instalador Beta 7 permanece PENDIENTE**.
